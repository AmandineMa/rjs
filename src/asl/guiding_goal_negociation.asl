...
+!guiding_goal_negociation(ID, Human,Place): true <-
	if(not jia.word_individual(findSub, Place, PlaceOnto)){
		if(jia.word_class(findSub, Place, Class1)){
...
//				jia.word_individual(findSub, ListF, PlaceOnto);
			...
			jia.word_individual(findFuzzy, Place, Fuzzy);
			if(.list(Fuzzy)){
				.nth(0,Fuzzy,F);
				jia.word_individual(findSub, F, PlaceOnto);
			}else{
				jia.word_individual(findSub, Fuzzy, PlaceOnto);
			}
			jia.verba_name(PlaceOnto, PlaceVerba);
